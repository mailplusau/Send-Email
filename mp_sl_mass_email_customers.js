...
function sendEmail(request, response) {
    ...
    if (request.getMethod() === "GET") {
        ...
        var mpexLetterTypeSearch = nlapiCreateSearch('customlist_mpex_pricing_letter_type', null, columns)
        var resultSetYLetterTypes = mpexLetterTypeSearch.runSearch();

        inlinehtml2 += '<div class="form-group container row_template">';
        inlinehtml2 += '<div class="row">'

        inlinehtml2 += '<div class="col-xs-8 letter_type_section"><div class="input-group"><span class="input-group-addon">Letter Type';

        inlinehtml2 += '<span class="mandatory">*</span>';

        inlinehtml2 += '</span><select  id="letter_type" class="form-control" ><option></option>';
        resultSetYLetterTypes.forEachResult(function(searchResultCampTemp) {

            var listId = searchResultCampTemp.getValue('internalId');
        ...
        form.addField('custpage_pdf', 'textarea', 'BODY').setDisplayType('hidden');
        ...
        form.setScript('customscript_cl_mass_email_customers');
        // form.addButton('pdfprint', 'Print Letters', 'onclick_print()');
        form.addSubmitButton('Send Email');

        response.writePage(form);
    } else {
        var letter_type = request.getParameter('custpage_letter_type');
        ...
        var pdf = request.getParameter('custpage_pdf');
        ...
        var reschedule_status = nlapiScheduleScript('customscript_ss_mass_email_customers', 'customdeploy1', params);
        ...
        nlapiSetRedirectURL('SUITELET', 'customscript_sl_sent_email', 'customdeploy1', null, params_progress);
    }
}
