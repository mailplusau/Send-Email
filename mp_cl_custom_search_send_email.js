...
function pageInit() {
    ...
    var search_dropdown = nlapiGetFieldValue('custpage_search_dropdown')


    if (!isNullorEmpty(search_dropdown) && search_dropdown != 0) {
        ...
        resultSetCustomer.forEachResult(function (searchResult) {
            ...
            dataSet += '{"cust_id":"' + custid + '", "entityid":"' + entityid +
                '", "companyname_text":"' + companyname + '", "company_name":"' +
                companyname + '"},';
...
$(document).on("change", ".search_dropdown", function (e) {

    var search_dropdown = $(this).val();

    var url = baseURL +
        "/app/site/hosting/scriptlet.nl?script=1076&deploy=1&sorts[customername]=1";

    url += "&search=" + search_dropdown + "";
